...
def load_sp500_universe():
    """
    Fetches S&P 500 constituents. Tries three methods in order:
      1. Wikipedia via pd.read_html (primary)
      2. Wikipedia via requests + lxml (backup parser)
      3. Hardcoded fallback list
    Stores the error reason so we can surface it in the UI.
    """
    import traceback

    # ── Method 1: pd.read_html with lxml ─────────────────
    ...
    try:
        ...
        if all([symbol_col, name_col, sector_col]):
            universe = []
            ...
            if len(universe) > 400:   # sanity check — S&P 500 should have 500+
    ...
    try:
        ...
        err_msg = f"Method 1: {type(e1).__name__} | Method 2: {type(e2).__name__}: {e2}"
        return FALLBACK_UNIVERSE, True, err_msg

    return FALLBACK_UNIVERSE, True, "Parsed but got fewer than 400 tickers — table structure may have changed"

# Initialise universe — loaded once per session
if "universe_loaded" not in st.session_state:
    _u, _fallback, _err = load_sp500_universe()
...
def render_app():
    ...
    with tab_screen:
        ...
        if st.session_state.get("universe_fallback"):
            err = st.session_state.get("universe_error","")
            err_detail = f"<br><span style='color:#6b6e6c;font-size:10px;'>{err}</span>" if err else ""
            st.markdown(f"""<div style="background:#1a1512;border:1px solid #c8953a44;border-left:3px solid #c8953a;
                ...
                ⚠ Wikipedia fetch failed — running on 52-stock fallback universe.{err_detail}</div>""",
                unsafe_allow_html=True)
        else:
